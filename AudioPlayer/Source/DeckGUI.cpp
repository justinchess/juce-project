...
void DeckGUI::resized()
{
    ...
    playButton.setBounds(0, 7 * getHeight() / 8, mainRight / 3, getHeight() / 8);
    stopButton.setBounds(mainRight / 3, 7 * getHeight() / 8, mainRight / 3, getHeight() / 8);
    loadButton.setBounds(2 * mainRight / 3, 7 * getHeight() / 8, mainRight / 3, getHeight() / 8);
...
void DeckGUI::buttonClicked(juce::Button* button)
{
    ...
    if (button == &loadButton)
    {
        ...
        {
            loadFile(juce::URL{ chooser.getResult() }, chooser.getResult().getFullPathName());
        }
    }
}
...
void DeckGUI::filesDropped(const juce::StringArray& files, int x, int y)
{
    ...
    if (files.size() == 1)
    {
        loadFile(juce::URL{ juce::File{files[0]} }, files[0]);
    }
}

void DeckGUI::loadFile(juce::URL audioURL, String path)
{
